...
def create_graphs_and_buckets():
    """
    Create graphs and buckets for citation analysis.
    Steps:
        1. Compute global stats (mean, std)
        2. Compute year-normalized z-scores
        3. Plot mean citations by year
        4. Plot year-normalized z-score distribution
        5. Plot tail histogram (0.33σ buckets)
        6. Plot log-scale raw citation distribution
        7. Create percentile buckets and plot distribution
        8. Create Lorenz curve and compute Gini coefficient
        9. Save enriched dataset with new columns
    """
    ...
    if "year_zscore" not in df.columns:
        print("[INFO] Computing year-normalized statistics...")
        ...
        print("[INFO] Year-normalized statistics already exist. Skipping.")
    ...
    if "global_z" not in df.columns:
        ...
        print("[INFO] global_z already exists. Skipping recalculation.")
    ...
    # ============================================================
    # 6️⃣ PERCENTILE BUCKETS (TOP 5%, 15%, 30%, 50%)
    # ============================================================

    if "percentile_bucket" not in df.columns:
        ...
        print("[INFO] percentile_bucket already exists. Skipping recalculation.")
    ...
    # ============================================================
    # SAVE ENRICHED DATASET (SAFE WRITE)
    # ============================================================

    print("[INFO] Writing updated dataset (no duplicate columns created).")
